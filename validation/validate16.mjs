// Phase 1.6 验收脚本：日常训练体验与数据逻辑修正（15 步行进 + 截图）
// 覆盖规范「十一、验收流程」（15 步）：
//  1 建档：目标体重默认为空
...
//  5 完成一组后进入休息计时
//  6 刷新：组数据与休息倒计时恢复
//  7 只完成 1/14 组 → 点击结束 → 出现未完成确认弹窗
//  8 保存为 partial
//  9 Summary 显示 计划14组/完成1组/完成率7%
// 10 Today 显示完成额外训练
// 11 肌群数据区分直接组 / 间接组
// 12 不出现 2.6kg（取整到器械增量）
// 13 历史中可以找到本次 partial 训练
// 14 训练历史可打开 partial workout 详情
// 15 刷新网站数据保留
